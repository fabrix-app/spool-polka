...
describe('polka options', () => {
  ...
  it('Should execute init method', () => {
    assert(global.app.spools.polka.server.initOk)
  })
      ...
      request
        ...
        .end((err, res) => {
          if (!err) {
            ...
            //FIXME update spool-router to have origin as string only
            //assert.equal(res.headers['access-control-allow-origin'], 'http://fabrix.app')
          }
          done(err)
        })
    })
