...
export const Server = {
  ...
  createServer(app: FabrixApp) {
    ...
    const polka = app.config.get('web.polka')

    if (!polka) {
      throw new Error('Polka Web Server was not in app.config.web.polka')
    }
    ...
    if (main.paths && main.paths.www) {
      // app.config.set('web.middlewares.www', polka.static(main.paths.www, {
      //   maxAge: this.webConfig.cache
      // }))
      this.middlewares.www = polka.static(main.paths.www, {
        maxAge: this.webConfig.cache
      })
    }
    else {
   ...
   * @param server polka server
   * @param app fabrix app
   */
  registerMiddlewares(app: FabrixApp, server: Polka) {
  ...
  /**
   * Register routes to polka server
   */
  registerRoutes(app: FabrixApp, server: Polka) {
    // Sort the routes so that they are always in the correct polka order.
    // Could be related to https://github.com/fabrix-app/spool-router/issues/6
    const routes = new Map([...app.routes].reverse())
    const polka = app.config.get('web.polka')
    const polkaRouter = polka.Router
    const router = polkaRouter()
    Utils.extendsPolkaRouter(router)
    ...
    each(this.serverRoutes, (route, path) => {
      ...
      if (isPlainObject(route.handler)) {
        if (route.handler.directory && route.handler.directory.path) {
          router.use(parts[1], polka.static(route.handler.directory.path))
        }
        else {
        ...
        // Format route to polka protocol, maybe `spool-router` will do this in the future
  ...
  start(app, server) {
    ...
    if (!init) {
      const err = new Error('Init is not define and polka can not start')
